...
export function run(method: string, args: any[], remote: Remote = fixture(), extraEnv: NodeJS.ProcessEnv = {}) {
  const child = fork(join(__dirname, '../dist/tests/worker.js'), [], {
    env: { ...process.env, ...extraEnv }, execArgv: extraEnv.CACHE_FIXED_NOW ? ['--require', join(__dirname, 'clock.cjs')] : [], stdio: ['ignore', 'pipe', 'pipe', 'ipc'],
  });
  children.add(child);
...
export function pauseSecondPage(products = [product('fresh'), product('keep', 'background')], error?: string) {
  const reached = deferred();
  const release = deferred();
  const remote: Remote = async (method, filters) => {
    if (method !== 'getProducts') return lifecycle(filters.productSkuIdList);
    if (filters.page === 1) return page([products[0]], 2);
    reached.resolve();
    await release.promise;
    if (error) throw new Error(error);
    return page([products[1]], 2);
  };
  return { remote, reached: reached.promise, release: () => release.resolve() };
}
